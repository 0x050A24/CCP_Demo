...
static inline uint8_t calculate_deadtime_value(uint32_t deadtime_ns,
                                               uint32_t timer_clk_hz);

void TIM0_PWM_Init(uint16_t prescaler,
                   uint32_t period,
                   uint32_t deadtime_ns)
{
    ...
    timer_initpara.prescaler        = prescaler;  // 0
    ...
    timer_initpara.repetitioncounter
        = 1;  // every 2 update events calls 1 interrupt
    ...
    brk_param.deadtime     = calculate_deadtime_value(
        deadtime_ns, SystemCoreClock);  // 2us
...
void cal_fmain(float* f, float* Ts, float* PWM_ARR)
{
    ...
    float counter_mode
        = ((TIMER_CTL0(TIMER0) & TIMER_CTL0_CAM) >> 5) == 0 ? 1.0F
                                                            : 2.0F;
