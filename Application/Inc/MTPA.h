...
/* ------------------ 可调宏/常量 ------------------ */
#define MTPA_TABLE_POINTS 51  // NOLINT(cppcoreguidelines-macro-to-enum)
#define MTPA_PSI_MAX      1.10f /* 内环磁链幅值上限（题目要求 0-1.1） */
#define MTPA_PSI_SCAN_STEPS \
    50  // NOLINT(cppcoreguidelines-macro-to-enum)
#define MTPA_PSI_BISECT_TOL \
    1e-5f                      /* 二分求解 Psi 的容忍（磁链或转矩） */
#define MTPA_TH_TOL      1e-4f /* 最终 gamma 搜索精度（弧度） */
#define MTPA_TH_MAX_ITER 80  // NOLINT(cppcoreguidelines-macro-to-enum)
#define MTPA_PSI_MIN     0.0f
#define MTPA_MIN_IS      1e-9f /* 防止除零 */

/* 极对数（用于转矩公式）与因子 */
#define POLE_PAIRS 2  // NOLINT(cppcoreguidelines-macro-to-enum)
