...
*/
static bool find_psi_for_T_at_gamma(float T_req, float gamma, float* out_psi, float* out_Id,
                                    float* out_Iq, float* out_Te, float* out_Is)
{
...
bool MTPA_compute_for_T(float T_req, MTPA_Point* out_p)
{
  ...
  int iter = 0;
  float best_gamma = 0.0f, best_psi = 0.0f, best_Id = 0.0f, best_Iq = 0.0f;
  ...
  while ((b - a) > MTPA_TH_TOL && iter < MTPA_TH_MAX_ITER)
  {
    if (Is_c < Is_d)
    {
      ...
      c = b - (b - a) * gr;
      /* eval c */
      bool ok = find_psi_for_T_at_gamma(T_req, c, &psi_tmp, &Id_tmp, &Iq_tmp, &Te_tmp,
                                        &psi_tmp /*reuse*/);
      if (ok)
      {
    ...
    else
    {
      ...
      Is_c = Is_d;
      d = a + (b - a) * gr;
      bool ok = find_psi_for_T_at_gamma(T_req, d, &psi_tmp, &Id_tmp, &Iq_tmp, &Te_tmp,
                                        &psi_tmp /*reuse*/);
      if (ok)
      {
  ...
  for (int k = 0; k <= Ncheck; ++k)
  {
    ...
    float psi_out = 0.0F, id_out = 0.0F, iq_out = 0.0F, te_out = 0.0F, is_out = 0.0F;
                       ...
                       float* Iq_out)
{
  ...
  {
    if (Iq_ref >= table[i].Iq && Iq_ref <= table[i + 1].Iq) break;
  }
