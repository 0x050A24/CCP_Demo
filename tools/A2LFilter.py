...
def filter_a2l(in_path, out_path):
    ...
    mblocks = [(s, e, name) for (kw, name, s, e) in blocks if kw in ('MEASUREMENT', 'CHARACTERISTIC')]
    ...
    while idx < len(lines):
        ...
        if stripped.startswith('/begin MODULE'):
            ...
            for dtype, (mname, fmt) in compu_map.items():
                out_lines.append(f"    /begin COMPU_METHOD {mname} \"Identity {dtype}\"\n")
        ...
        if block:
            ...
            for i in range(s, e + 1):
                m = re.match(r"(\s*)(" + "|".join(compu_map.keys()) + r")\s+NO_COMPU_METHOD(.*)", lines[i])
                if m:
                    dtype = m.group(2)
                    rest  = m.group(3)
...
if __name__ == '__main__':
  ...
  filter_a2l.py -i input.a2l -o output.a2l''')
    parser.add_argument('-i', '--input',  dest='input', required=True, help='Input A2L file path')
    parser.add_argument('-o', '--output', dest='output', required=True, help='Output A2L file path')
